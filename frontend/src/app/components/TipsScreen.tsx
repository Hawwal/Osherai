...
export function TipsScreen({ tips = [], onExplainTip }: Props) {
  ...
  const openArticle = async (tip: typeof items[number]) => {
    ...
    try {
      const prompt = `Write a practical, concise financial tips article about "${tip.title}" for an Osher AI user in Nigeria or Africa. Tie it to savings discipline, avoiding impulse spending, stablecoin savings, local currency thinking, and simple next actions. Do not use jargon.`;
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto pb-28" style={{ background: '#f5f5fb', scrollbarWidth: 'none' }}>
      <div className="px-5 pt-12 pb-4"><h1 className="font-display" style={{ fontSize: '1.6rem', fontWeight: 800, color: '#0d0d14', letterSpacing: '-0.02em' }}>Financial Tips</h1><p style={{ fontSize: '0.82rem', color: '#9a9ab8', marginTop: 3 }}>Practical coaching for stronger savings habits</p></div>
