  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto pb-28" style={{ background: '#f5f5fb', scrollbarWidth: 'none' }}>
      <div className="px-5 pt-12 pb-4 flex items-center justify-between">
        <div><h1 className="font-display" style={{ fontSize: '1.6rem', fontWeight: 800, color: '#0d0d14', letterSpacing: '-0.02em' }}>Savings Goals</h1><p style={{ fontSize: '0.82rem', color: '#9a9ab8', marginTop: 3 }}>Keep important money separate from everyday spending.</p></div>
      ...
      <div className="px-5 flex flex-col gap-4">
        <button onClick={onAskAi} className="rounded-3xl p-4 flex items-center gap-3 text-left" style={{ background: '#fff', color: '#0d0d14', boxShadow: '0 3px 12px rgba(0,0,0,0.06)' }}>
          ...
          <span className="flex-1"><span className="block font-display" style={{ fontWeight: 800, fontSize: '0.96rem' }}>Ask Osher AI to shape the habit</span><span className="block" style={{ color: '#6b6b8a', fontSize: '0.78rem', lineHeight: 1.45, marginTop: 2 }}>Describe what you want to protect, the amount, and your deadline.</span></span>
        </button>

        {goals.length === 0 && (
          <div className="rounded-3xl p-5" style={{ background: '#fff', boxShadow: '0 3px 12px rgba(0,0,0,0.06)' }}>
            <p className="font-display" style={{ fontWeight: 800, color: '#0d0d14', fontSize: '1.05rem' }}>No protected goals yet</p>
            <p style={{ fontSize: '0.85rem', color: '#6b6b8a', lineHeight: 1.6, marginTop: 6 }}>Use AI Chat to say something like “Help me protect ₦500,000 for rent by December.”</p>
          </div>
        )}
            ...
            <div key={goal.id} className="rounded-3xl overflow-hidden" style={{ background: '#fff', boxShadow: '0 3px 12px rgba(0,0,0,0.06)' }}>
              <button onClick={() => onGoalClick(goal)} className="w-full text-left p-5">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span style={{ fontSize: '0.68rem', fontWeight: 700, color: onChainReady ? '#2d7a47' : '#b36a00', background: onChainReady ? '#e8f5ec' : '#fff3dc', padding: '4px 8px', borderRadius: 7 }}>{onChainReady ? 'Protected' : 'Setup'}</span>
                </div>
